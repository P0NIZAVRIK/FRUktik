...
class AuthService extends ChangeNotifier {
  ...
  Future<void> _initAuth() async {
    ...
    // 1. Always load local user first for immediate UI state (offline support)
    ...
    if (SupabaseConfig.isConfigured) {
      ...
      if (session != null) {
        // User is logged in, refresh profile in background
        // We don't await this to keep UI responsive if cache exists
        _fetchProfile(session.user.id, session.user.email!);
      } else {
        // Supabase says we are logged out
        if (_currentUser != null) {
          // Verify if we really should clear it (e.g. token expired)
          // For now, if no session, we assume logout
          _currentUser = null;
        }
      }
      ...
      Supabase.instance.client.auth.onAuthStateChange.listen((data) {
        ...
        if (event == AuthChangeEvent.signedIn && session != null) {
          ...
          _currentUser = null;
          SharedPreferences.getInstance().then((p) => p.remove('user_data'));
          notifyListeners();
        }
      });
    }
  ...
  Future<void> _loadLocalUser() async {
    try {
      ...
      if (userData != null) {
        ...
        debugPrint('👤 User loaded from local cache: ${_currentUser?.email}');
      }
    } catch (e) {
    ...
    try {
      ...
      if (SupabaseConfig.isConfigured) {
        ...
        if (dailyCalorieTarget != null) {
          updates['daily_cal_target'] = dailyCalorieTarget;
        }
